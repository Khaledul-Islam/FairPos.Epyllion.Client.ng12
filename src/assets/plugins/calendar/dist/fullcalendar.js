...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function getContentRect(el, origin) {
    ...
    var left =
      offset.left + getCssFloat(el, 'border-left-width') + getCssFloat(el, 'padding-left') - (origin ? origin.left : 0);
    var top =
      offset.top + getCssFloat(el, 'border-top-width') + getCssFloat(el, 'padding-top') - (origin ? origin.top : 0);
...
*/
  ...
  function TaskQueue(debounceWait) {
    ...
    this.add = typeof debounceWait === 'number' ? debounce(addTask, debounceWait) : addTask; // potentially debounce, for the public method // if not a number (null/undefined/false), no debounce at all
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  Grid.mixin({
    ...
    getSegDefaultBackgroundColor: function (seg) {
      ...
      return (
        source.backgroundColor || source.color || this.view.opt('eventBackgroundColor') || this.view.opt('eventColor')
      );
    },
  ...
  var DayGrid = (FC.DayGrid = Grid.extend(DayTableMixin, {
    ...
    renderDayRowHtml: function (row, isRigid) {
      ...
      return (
        ...
        '<table>' +
        (this.numbersVisible ? '<thead>' + this.renderNumberTrHtml(row) + '</thead>' : '') +
        '</table>' +
        '</div>' +
        '</div>'
      );
  ...
  DayGrid.mixin({
    ...
    fgSegHtml: function (seg, disableResizing) {
      ...
      var isResizableFromStart =
        !disableResizing && event.allDay && seg.isStart && view.isEventResizableFromStart(event);
...
*/
  ...
  FC.datepickerLocale = function (localeCode, dpLocaleCode, dpOptions) {
    ...
    if ($.datepicker) {
      ...
      $.datepicker.regional[dpLocaleCode] = $.datepicker.regional[localeCode] = dpOptions; // alias
...
-----------------------------------------------------------------------------------------*/
  ...
  Calendar.prototype.isSelectionSpanAllowed = function (span) {
    return (
      this.isSpanAllowed(span, this.options.selectConstraint, this.options.selectOverlap) &&
