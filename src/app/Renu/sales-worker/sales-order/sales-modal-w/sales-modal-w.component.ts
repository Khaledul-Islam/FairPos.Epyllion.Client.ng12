...
@Component({
  selector: 'app-sales-modal-w',
  templateUrl: './sales-modal-w.component.html',
  styleUrls: ['./sales-modal-w.component.scss'],
})
export class SalesModalWComponent implements OnInit {
  ...
  async SaveSalesOrder() {
    ...
    if (this.TempSalesOrderList.length == 0) {
      this.alertService.warning('No item Added');
      return;
    }
    ...
    let promise = this.http
      .post(this.env.apiUrl + 'SalesWorker/SalesOrderSave', this.TempSalesOrderList, { responseType: 'text' })
      .toPromise();
    return promise
      .then((resp) => {
  ...
  async TempSalesOrderSave(buy: Buy) {
    ...
    let promise = this.http.post(this.env.apiUrl + 'SalesWorker/TempSalesOrderSave', buy).toPromise();
    return promise
      .then((resp) => {
  ...
  async selectProduct(ID: any) {
    ...
    if (this.buyList.length <= 0) {
      this.alertService.warning("This product isn't received yet . No Product Found");
    }
  }
  ...
  async GetProductsByProductID(prdId: any) {
    let promise = this.http.get(this.env.apiUrl + 'SalesWorker/GetProductsByProductID?PrdId=' + prdId).toPromise();
    return promise.then((resp) => {
