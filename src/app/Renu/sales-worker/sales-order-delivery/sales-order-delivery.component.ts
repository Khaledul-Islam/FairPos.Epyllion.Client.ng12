...
@Component({
  selector: 'app-sales-order-delivery',
  templateUrl: './sales-order-delivery.component.html',
  styleUrls: ['./sales-order-delivery.component.scss'],
})
export class SalesOrderDeliveryComponent implements OnInit {
  ...
  async GetPendingItemsByEmp(empid: any) {
    ...
    return promise.then((resp) => {
      ...
      return this.orderList.find((p) => p.SO_NO == this.so_no && p.SalesStockType == 'Worker Stock');
    });
  }
  ...
  async SaveAPICall() {
    ...
    let promise = this.http
      .post(this.env.apiUrl + 'SalesOrderPrint/SalesOrderDeliverySave', this.orderDetailsList, { responseType: 'text' })
      .toPromise();
    return promise
      .then((resp) => {
        this.Employee = new Employee();
