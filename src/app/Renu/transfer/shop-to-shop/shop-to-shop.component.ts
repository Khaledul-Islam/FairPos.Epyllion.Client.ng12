...
@Component({
  selector: 'app-shop-to-shop',
  templateUrl: './shop-to-shop.component.html',
  styleUrls: ['./shop-to-shop.component.scss'],
})
export class ShopToShopComponent implements OnInit {
  ...
  ShopListDDL() {
    this.http.get(this.env.apiUrl + 'api/ShopList').subscribe(
      (result) => {
        ...
        this.shopList = data.filter((item: { ShopID: string | undefined }) => {
          return item.ShopID != this.ShopID;
        });
      },
      (error) => {
  ...
  async SaveTempStockTransfer() {
    ...
    let promise = this.http.post(this.env.apiUrl + 'ShopToShop/SaveTempStockTransfer', this.buyDetails).toPromise();
    return promise
      .then((res) => {
        this.rerender();
        return res;
  ...
  async saveButtonClick() {
    ...
    this.alertService.confirm('are you sure you want to Save this data', () => {
      let promise = this.http
        .post(this.env.apiUrl + 'ShopToShop/SaveStockTransfer', this.TempStockTransferList, { responseType: 'text' })
        .toPromise();
      return promise
        .then((res) => {
          this.buyDetails = new Buy();
  ...
  LoadDataTable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        dataTablesParameters.search.vendorno = 'Shop';
        that.http
          .post<DataTablesResponse>(this.env.apiUrl + 'ShopToShop/GetTempDataTable', dataTablesParameters, {})
          .subscribe((resp) => {
