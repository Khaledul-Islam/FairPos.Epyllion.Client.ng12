...
import { TransferModalComponent } from '../mainstock-to-staff-worker/transfer-modal/transfer-modal.component';

@Component({
  selector: 'app-worker-to-main-staff',
  templateUrl: './worker-to-main-staff.component.html',
  styleUrls: ['./worker-to-main-staff.component.scss'],
})
export class WorkerToMainStaffComponent implements OnInit {
  ...
  public APIURL = {
    parent: 'WorkerToMainStaff/GetWorkerStaffProducts',
    child: 'WorkerToMainStaff/GetWorkerStaffNewBarcode',
    details: 'WorkerToMainStaff/GetWorkerStaffBarcodeExp',
  };
  ...
  async GetTransferBarcodeExp(Barcode: any) {
    let promise = this.http
      .get(this.env.apiUrl + 'WorkerToMainStaff/GetWorkerStaffBarcodeExp?expbarcode=' + Barcode)
      .toPromise();
  ...
  async TransferModal(sBarcode: any) {
    if (sBarcode) {
      ...
      await this.GetTransferBarcodeExp(sBarcode);
    } else {
      if (this.buyDetails.SupID === null || this.buyDetails.SupID === undefined) {
        this.alertService.warning('Supplier is Required');
        return;
  ...
  async SaveTempStockTransfer() {
    ...
    let promise = this.http
      .post(this.env.apiUrl + 'WorkerToMainStaff/SaveTempStockTransfer', this.buyDetails)
      .toPromise();
    return promise
      .then((res) => {
        this.rerender();
  ...
  async saveButtonClick() {
    ...
    this.alertService.confirm('are you sure you want to Save this data', () => {
      let promise = this.http
        .post(this.env.apiUrl + 'WorkerToMainStaff/SaveStockTransfer', this.TempStockTransferList, {
          responseType: 'text',
        })
        .toPromise();
      return promise
  ...
  LoadDataTable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        dataTablesParameters.search.vendorno = 'Worker Stock';
        that.http
          .post<DataTablesResponse>(this.env.apiUrl + 'WorkerToMainStaff/GetTempDataTable', dataTablesParameters, {})
          .subscribe((resp) => {
